...
if not os.path.exists('plots_lightgbm_150000'):
    os.makedirs('plots_lightgbm_150000')
    print("'plots_lightgbm_150000' folder created.")
...
for feature in feature_columns:
    ...
    plt.savefig(f'plots_lightgbm_150000/distribution_{feature}.png')
    plt.close()
...
plt.savefig('plots_lightgbm_150000/feature_correlation_matrix.png')
plt.close()
...
for name, X_resampled, y_resampled in [("SMOTEENN", X_resampled_smoteenn, y_resampled_smoteenn),
    ...
    random_search = RandomizedSearchCV(estimator=lgb_model, param_distributions=lgb_param_distributions, 
                                       n_iter=30, cv=5, n_jobs=-1, scoring='roc_auc', random_state=42, verbose=0)
    ...
    print(f"\nAUC-ROC Score for LightGBM with {name}:")
    ...
    plt.savefig(f'plots_lightgbm_150000/confusion_matrix_LightGBM_{name}.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'plots_lightgbm_150000/feature_importance_LightGBM_{name}.png')
    plt.close()
    ...
    plt.legend(loc='best')
    plt.savefig(f'plots_lightgbm_150000/learning_curves_LightGBM_{name}.png')
    plt.close()

print("\nVisualizations have been saved in the 'plots_lightgbm_150000' folder.")
print("\nModeling completed.")