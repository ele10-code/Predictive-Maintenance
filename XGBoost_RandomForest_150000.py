...
if not os.path.exists('plots_advanced_resampling_xgboost'):
    os.makedirs('plots_advanced_resampling_xgboost')
    print("'plots_advanced_resampling_xgboost' folder created.")
...
# Train and evaluate models for both resampling techniques and both algorithms
for name, X_resampled, y_resampled in [("SMOTEENN", X_resampled_smoteenn, y_resampled_smoteenn),
    ...
    for model_name, model, param_distributions in [
        ("RandomForest", RandomForestClassifier(random_state=42, n_jobs=-1), rf_param_distributions),
        ("XGBoost", xgb.XGBClassifier(random_state=42,  eval_metric='logloss'), xgb_param_distributions)
    ]:
        print(f"\nTraining {model_name} with {name}...")
        random_search = RandomizedSearchCV(estimator=model, param_distributions=param_distributions, 
                                           n_iter=30, cv=5, n_jobs=-1, scoring='roc_auc', random_state=42)
        random_search.fit(X_resampled, y_resampled)

        print(f"Best parameters for {model_name} with {name}:", random_search.best_params_)
        ...
        print(f"\nClassification Report for {model_name} with {name}:")
        print(classification_report(y_test, y_pred))

        print(f"\nAUC-ROC Score for {model_name} with {name}:")
        print(roc_auc_score(y_test, y_pred_proba))

        # Confusion Matrix
        plt.figure(figsize=(10, 8))
        sns.heatmap(confusion_matrix(y_test, y_pred), annot=True, fmt='d', cmap='Blues')
        plt.title(f'Confusion Matrix - {model_name} with {name}')
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.savefig(f'plots_advanced_resampling_xgboost/confusion_matrix_{model_name}_{name}.png')
        plt.close()
        ...
        plt.title(f'Feature Importance - {model_name} with {name}')
        plt.tight_layout()
        plt.savefig(f'plots_advanced_resampling_xgboost/feature_importance_{model_name}_{name}.png')
        plt.close()

print("\nVisualizations have been saved in the 'plots_advanced_resampling_xgboost' folder.")
print("\nModeling completed.")
